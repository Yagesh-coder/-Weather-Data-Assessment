...
public class WeatherServiceImpl implements WeatherService {
	...
	public void loadCsv(String path) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				WeatherRecord w = WeatherRecord.builder().timestamp(c[0]).condition(c[1]).temperature(parse(c[11]))
						.humidity(parse(c[6])).pressure(parse(c[8])).windSpeed(parse(c[19])).build();
	...
	public List<WeatherRecord> filter(Double min, Double max, String cond) {
		...
		return all.stream().filter(w -> min == null || w.getTemperature() != null && w.getTemperature() >= min)
